...
- PRICE_VARIANCE: Invoiced price ≠ PO agreed price
- QTY_MISMATCH: Invoiced quantity ≠ received quantity
- PHANTOM_LINE: Billed item has NO corresponding PO entry
- financial_impact MUST be calculated for every anomaly as the total USD delta.
- Every anomaly MUST include the relevant 'sku' property.
- Emit a 'MATCHED_TO_PO' relation between InvoiceLine and PurchaseOrder with properties: {method, score}.
- Emit an 'EVIDENCE_FROM' relation from every anomaly to the SourceDocument.
